...
const megaman = {
  ...
  move(pos: { x: number; y: number }) {
    ...
    this.pos.x = Math.floor(this.pos.x + x + 0.5);
    this.pos.y = Math.floor(this.pos.y + y + 0.5);

    this.collider.x = Math.floor(this.collider.x + x + 0.5);
    this.collider.y = Math.floor(this.collider.y + y + 0.5);
  },
};
